...
class SpatialCrossAttentionPts(BaseModule):
    ...
    def __init__(self,
        ...
        self.deformable_attention = MSDeformableAttention3DPts(**deformable_attention)
        self.embed_dims = embed_dims
        self.num_cams = num_cams
        self.output_proj = nn.Linear(embed_dims, embed_dims)
        self.batch_first = batch_first
        self.cross_modal_feat = cross_modal_feat
        if self.cross_modal_feat:
            self.deformable_cross_modal_attention = MSDeformableAttention3DImg(**deformable_attention)
        self.init_weights()
    ...
    def forward(self,
        ...
        if self.cross_modal_feat:
            cross_modal_feat = cross_modal_feat.permute(1, 0, 2)
            cross_modal_queries = self.deformable_cross_modal_attention(query=query,
                                                key=cross_modal_feat,
                                                value=cross_modal_feat,
                                                reference_points=reference_points_lidar,
                                                spatial_shapes=spatial_shapes,
                                                level_start_index=level_start_index)
            queries = torch.add(queries, cross_modal_queries)
...
class MSDeformableAttention3DImg(MSDeformableAttention3DPts):
    """An attention module used in BEVFormer based on Deformable-Detr.
    `Deformable DETR: Deformable Transformers for End-to-End Object Detection.
    <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None):
        super(MSDeformableAttention3DImg, self).__init__(embed_dims, num_heads, num_levels, num_points)
        self.value_proj = nn.Linear(80, embed_dims)