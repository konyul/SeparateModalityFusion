...
class CMTransformerDecoderLayer(DetrTransformerDecoderLayer):
    ...
    def __init__(self,
        ...
        if self.hybrid_query:
            self.self_posembedv1 = PositionEncodingLearned(**pos_encoding_cfg)
            ...
            self.self_posembedv1 = PositionEncodingLearned(**pos_encoding_cfg)
        self.cross_posembedv1 = PositionEncodingLearned(**pos_encoding_cfg)
    ...
    def forward(self,
        ...
        #     Tensor: forwarded results, has shape (bs, num_queries, dim).
        # """
        if self.self_posembedv1 is not None and query_pos is not None:
            if self.hybrid_query:
                f_query_pos = self.self_posembedv1(query_pos[0]).transpose(1, 2)
                i_query_pos = self.self_posembedv2(query_pos[1]).transpose(1, 2)
                p_query_pos = self.self_posembedv3(query_pos[2]).transpose(1, 2)
            else:
                f_query_pos = self.self_posembedv1(query_pos).transpose(1, 2)
        else:
            query_pos = None
        if self.cross_posembedv1 is not None and key_pos is not None:
            f_key_pos = self.cross_posembedv1(key_pos).transpose(1, 2)
